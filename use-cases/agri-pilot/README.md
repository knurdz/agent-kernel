<p align="center">
  <img src="docs/branding/agripilot-icon.png" alt="AgriPilot logo" width="160" />
</p>
...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="docs/architecture/agripilot.architecture.html">Architecture</a>
  &nbsp;·&nbsp;
  <a href="mobile/README.md">Mobile dev</a>
</p>

<p align="center">
  <em>Part of the <a href="https://github.com/yaalalabs/agent-kernel">Agent Kernel</a> monorepo · <code>use-cases/agri-pilot/</code></em>
</p>
...
<p align="center">
  <a href="docs/architecture/agripilot.architecture.html">
    <img src="docs/architecture/agripilot.architecture.png" alt="AgriPilot runtime architecture" width="900" />
  </a>
</p>

<p align="center"><em>Click for the interactive diagram (Archify). Source: <code>docs/architecture/agripilot.architecture.json</code></em></p>
...
Supervisor routing lives in [`agents/supervisor.py`](agents/supervisor.py). Marketplace order/dispatch logic is deterministic REST in `marketplace/order_service.py`, `dispatch_service.py`, and `tracking_service.py`.
...
Defined in [`pyproject.toml`](pyproject.toml). Production Docker reinstalls monorepo `ak-py` because PyPI `0.8.1` predates `auth.authoriser` and mobile thread routes — see [`deploy/Dockerfile`](deploy/Dockerfile).

### Entry points

| File | Role |
|------|------|
| [`demo.py`](demo.py) | CLI — `LangGraphModule([triage_agent])` for local testing |
| [`app.py`](app.py) | Production — REST + WhatsApp + Telegram + marketplace routers |
...
- **Supervisor** (`agents/supervisor.py`) — `langgraph_supervisor` triage with 20+ tools (marketplace, delivery, plan, profile)
...
Tools are wrapped with `@guarded` (`tools/tool_guard.py`) — per-session call limits and timeouts.
...
Config in [`config.yaml`](config.yaml); override with `AK_*` env vars (`AK_SESSION__TYPE=redis`, etc.). Docker and VPS use Redis for sessions, multimodal attachments, and conversation threads.
...
See [`mobile/README.md`](mobile/README.md) for Firebase push (optional) and physical-device LAN URLs.
...
| Path | Role |
|------|------|
| [`deploy/docker-compose.vps.yml`](deploy/docker-compose.vps.yml) | Production Compose stack |
| [`deploy/Caddyfile`](deploy/Caddyfile) | Automatic HTTPS reverse proxy |
| [`deploy/Dockerfile`](deploy/Dockerfile) | Hardened app image (non-root, baked knowledge ingest) |
| [`deploy/.env.production.example`](deploy/.env.production.example) | Documented env template |
| [`deploy/deploy-vps.sh`](deploy/deploy-vps.sh) | Idempotent deploy + operations |
...
Key knobs (see [`.env.local.example`](.env.local.example)):
...
Bound to the supervisor via `tools/marketplace_tools.py` and `tools/delivery_tools.py` — all `@guarded`. Examples:
...
├── docs/architecture/ # Archify runtime diagram (JSON, HTML, PNG)
...
<p align="center">
  <img src="docs/branding/agripilot-icon.png" alt="AgriPilot" width="48" /><br/>
  <strong>AgriPilot</strong><br/>
  Part of the <a href="https://github.com/yaalalabs/agent-kernel">Agent Kernel</a> monorepo · See repository root <code>LICENSE</code>.
</p>
